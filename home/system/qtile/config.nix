...
keys = [
    ...
    Key([mod], "j", lazy.spawn("bolt-launcher"), desc="Launch bolt launcher"),
    ...
    Screen(
        top=bar.Bar(
            [
                ...
                widget.Spacer(),
                widget.Mpris2(format="󰝚   {xesam:artist}  -  {xesam:title}", mouse_callbacks={"Button1": lambda: qtile.cmd_spawn("youtube-music")}),
                separator(),
                widget.WidgetBox(
                    fmt="  ",
